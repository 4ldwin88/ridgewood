...
begin
 ...
 else if ps.status not in ('active','held') then raise exception 'inactive_project_state_required_restore'; end if; if nullif(trim(reason_input),'') is null then raise exception 'disposition_reason_required'; end if; next_status:=case disposition_input when 'decline' then 'declined' else 'withdrawn_lost' end;
 end if;
 update public.project_states set status=next_status,updated_at=now() where id=ps.id returning * into ps;
 insert into public.project_state_disposition_events(workspace_id,project_state_id,event_type,new_status,prior_stage,reason,basis,actor_user_id) values(ps.workspace_id,ps.id,disposition_input,next_status,ps.stage,nullif(trim(reason_input),''),nullif(trim(basis_input),''),uid);
 return ps;
end $$;
