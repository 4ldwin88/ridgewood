...
export function AppShell({ session }: { session: Session }) {
  ...
  const [theme, setTheme] = useState<'light'|'dark'>(() => { try { return localStorage.getItem('ridgewood-theme') === 'dark' ? 'dark' : 'light'; } catch { return 'light'; } });
  useEffect(() => { try { localStorage.setItem('ridgewood-theme', theme); } catch { /* Preference still works for this session. */ } }, [theme]);
  ...
  const content = page === 'Business' ? <BusinessWorkspace initialProjectId={selectedProject}/> : page === 'Projects' ? <ProjectsWorkspace/> : page === 'Home' ? <PortfolioWorkspace onOpen={(id, business) => { setSelectedProject(business ? id : undefined); setPage(business ? 'Business' : 'Projects'); }}/> : <EmptyFoundation page={page}/>;
  ...
  return <div className="app-frame" data-theme={theme}>
    ...
    <main id="main-workspace" className="workspace"><div className="page-heading"><div><h1>{pageTitle}</h1></div><span className="demo-label">Demo · awaiting acceptance</span></div>{content}</main>
