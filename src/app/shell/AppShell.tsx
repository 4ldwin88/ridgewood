...
import { FormsWorkspace } from '../../modules/forms/FormsWorkspace';

type Page = 'Home' | 'Business' | 'Forms' | 'Projects' | 'Network' | 'More';
const pages: Page[] = ['Home', 'Business', 'Forms', 'Projects', 'Network', 'More'];
...
export function AppShell({ session }: { session: Session }) {
  ...
  const pageTitle = page === 'Business' ? 'Business pipeline' : page === 'Forms' ? 'Project State forms' : page;
  const content = page === 'Business' ? <BusinessWorkspace/> : page === 'Forms' ? <FormsWorkspace/> : <EmptyFoundation page={page}/>;
  return <div className="app-frame"><header className="topbar"><img src={`${import.meta.env.BASE_URL}assets/ridgewood-horizontal-light.svg`} alt="Ridgewood"/><div className="top-actions"><span className="version">{APP_VERSION}</span>{feedbackEnabled ? <DevNotesButton observability={developmentObservability} pagePath={pagePath} pageTitle={page}/> : null}<button type="button" onClick={signOut} disabled={signingOut}>{signingOut ? 'Exiting…' : 'Exit'}</button></div></header><nav className="primary-nav" aria-label="Primary">{pages.map((item) => <button key={item} className={page === item ? 'active' : ''} onClick={() => setPage(item)}>{item}</button>)}</nav><main className="workspace"><p className="eyebrow">{page}</p><h1>{pageTitle}</h1>{content}</main><footer><span>{session.user.email}</span><span>Ridgewood OS · {APP_VERSION}</span></footer></div>;
}
