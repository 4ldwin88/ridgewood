...
export function StrongVerificationPanel() {
  ...
  async function enroll() {
    ...
    try { const enrollment = await beginTotpEnrollment(state?.unverifiedFactorId); const next: StrongVerificationState = { currentLevel: state?.currentLevel ?? 'aal1', nextLevel: 'aal2', verifiedFactorId: state?.verifiedFactorId, enrollment }; setState(next); publish(next); }
