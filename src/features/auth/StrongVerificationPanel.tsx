...
export function StrongVerificationPanel() {
  ...
  async function enroll() {
    ...
    try {
      const enrollment = await beginTotpEnrollment(state?.unverifiedFactorId);
  ...
  async function copySecret() {
    if (!state?.enrollment?.secret) return;
    try { await navigator.clipboard.writeText(state.enrollment.secret); setCopied(true); }
    catch { setError('Could not copy the setup key. Press and hold the key to copy it manually.'); }
  }
  ...
  async function verify(event: FormEvent) {
    ...
    try { await verifyStrongFactor(factorId, code); setCode(''); await refresh(); }
    catch (caught) { setError(message(caught)); }
    finally { setBusy(false); }
  }

  const verified = state?.currentLevel === 'aal2';
  return <div className="verification-control verification-inline">
    <section className="verification-popover" aria-label="Strong verification">
      <strong>{verified ? 'Strong verification active' : 'Verify identity'}</strong>
      {verified ? <p>This session is AAL2 and can execute governed Project Authorization, subject to business authority.</p> : <>
        <p>Project Authorization requires a second factor. This confirms identity; it does not grant business authority.</p>
        {state?.verifiedFactorId ? <p>Open your authenticator app and enter the current six-digit Ridgewood OS code below.</p> : state?.enrollment ? <div className="verification-enrollment"><p>On this phone, add an account manually in your authenticator app and use this setup key. The QR code is only for another device.</p><div className="setup-key"><code>{state.enrollment.secret}</code><button type="button" className="secondary" onClick={() => void copySecret()}>{copied ? 'Copied' : 'Copy setup key'}</button></div><details><summary>Show QR code for another device</summary><img src={state.enrollment.qrCode} alt="Authenticator enrollment QR code"/></details></div> : <><p>{state?.unverifiedFactorId ? 'An incomplete Ridgewood OS authenticator setup was found. Restarting setup will safely replace that incomplete factor.' : 'No authenticator is set up yet.'}</p><button type="button" disabled={busy} onClick={() => void enroll()}>{busy ? 'Preparing…' : state?.unverifiedFactorId ? 'Restart authenticator setup' : 'Set up authenticator'}</button></>}
