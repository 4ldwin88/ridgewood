...
const dbToDomain: Record<string, ReadinessState> = { not_started:'not_started', in_progress:'in_progress', satisfied:'ready', blocked:'blocked', unknown:'not_started' };
const domainToDb: Record<ReadinessState,string> = { not_started:'not_started', in_progress:'in_progress', ready:'satisfied', blocked:'blocked' };
...
export async function listPredevelopmentDomains(opportunityId:string):Promise<PredevelopmentDomainState[]>{
  ...
  return (data??[]).map((r:any)=>({opportunityId:r.opportunity_id,domain:r.domain_key as PredevelopmentDomain,readiness:dbToDomain[r.readiness]??'not_started',summary:r.notes??undefined,blockers:[],unknowns:[],updatedAt:r.updated_at}));
}
export async function updatePredevelopmentDomain(opportunityId:string,domain:PredevelopmentDomain,readiness:ReadinessState,notes:string):Promise<void>{
  const {data:userData}=await supabase.auth.getUser(); const uid=userData.user?.id; if(!uid) throw new Error('Authentication required.');
  const {error}=await supabase.from('predevelopment_domains').update({readiness:domainToDb[readiness],notes:notes.trim()||null,updated_by:uid,updated_at:new Date().toISOString()}).eq('opportunity_id',opportunityId).eq('domain_key',domain); if(error) throw error;
}
