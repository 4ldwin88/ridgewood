...
export const supabaseQualificationRepository={
 async list(projectStateId:string):Promise<QualificationFinding[]>{await context();const {data,error}=await supabase.from('opportunity_qualification_findings').select('area,assessment,note').eq('project_state_id',projectStateId);if(error)throw error;return(data??[]) as QualificationFinding[]},
 async save(projectStateId:string,finding:QualificationFinding):Promise<void>{const {userId,workspaceId}=await context();const {error}=await supabase.from('opportunity_qualification_findings').upsert({workspace_id:workspaceId,project_state_id:projectStateId,area:finding.area,assessment:finding.assessment,note:finding.note??null,assessed_by:userId,assessed_at:new Date().toISOString(),updated_at:new Date().toISOString()},{onConflict:'project_state_id,area'});if(error)throw error},
