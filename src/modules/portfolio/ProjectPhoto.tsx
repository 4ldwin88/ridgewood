...
export function ProjectPhoto({project,editable=false,menuActions}:{project:ProjectState;editable?:boolean;menuActions?:ReactNode}) {
  ...
  return <div className={`project-photo${editable?' is-editable':''}`}>
    ...
    {menuActions ? <details className="row-menu photo-menu"><summary aria-label={`Manage ${project.name}`}><span aria-hidden="true">⋮</span></summary><div className="row-menu__items">{editable?<label className="photo-upload">{busy?'Uploading…':url?'Change photo':'Add photo'}<input aria-label={`Upload photo for ${project.name}`} type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={e=>{void upload(e.target.files?.[0]);e.target.value='';}}/></label>:null}{menuActions}</div></details> : editable?<label className="photo-upload">{busy?'Uploading…':url?'Change photo':'Add photo'}<input aria-label={`Upload photo for ${project.name}`} type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={e=>{void upload(e.target.files?.[0]);e.target.value='';}}/></label>:null}
