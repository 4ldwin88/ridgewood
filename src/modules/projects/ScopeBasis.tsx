...
function ScopeEditor({projectStateId,onSaved}:{projectStateId:string;onSaved:()=>void}){
 ...
 <label>Responsible party {['interface','owner_supplied'].includes(item.classification)?required:optional}<select value={item.partyId} onChange={e=>change(item.id,{partyId:e.target.value})}><option value="">Unassigned</option>{state.parties.map(p=><option key={p.id} value={p.id} disabled={p.retired&&p.id!==item.partyId}>{p.label}{p.retired?' · Removed from new selections':''}</option>)}</select><small>Required for interfaces and owner-supplied items. Select the existing party for assigned delivery scope. Proposed assignments do not award a subcontract.</small></label>
