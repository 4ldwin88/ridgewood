...
function ScopeEditor({projectStateId,onSaved}:{projectStateId:string;onSaved:()=>void}){
 ...
 <div className="setup-actions"><button disabled={!state.canEdit||busy||pending.current?.kind==='request'} onClick={()=>void command('save')}>{pending.current?.kind==='save'?'Retry scope save':'Save scope preparation'}</button><button disabled={busy} onClick={()=>void reload()}>Reload saved scope</button></div>
