...
create policy "users insert own development notes"
on public.development_notes for insert to authenticated
with check ((select auth.uid()) = user_id);

create policy "users read own development notes"
on public.development_notes for select to authenticated
...
create policy "users insert own development telemetry"
on public.development_telemetry for insert to authenticated
with check ((select auth.uid()) = user_id);

create policy "users read own development telemetry"
on public.development_telemetry for select to authenticated
using ((select auth.uid()) = user_id);
