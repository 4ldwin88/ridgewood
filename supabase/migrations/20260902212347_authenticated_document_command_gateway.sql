...
begin
  ...
  if command_name = 'createDraft' then
    ...
    result_id := private.create_document_draft_atomic(
      target_workspace,
      nullif(command_input->>'projectId','')::uuid,
      nullif(command_input->>'opportunityId','')::uuid,
      command_input->>'packageKey', command_input->>'categoryKey', command_input->>'documentType', command_input->>'title',
      coalesce(command_input->'initialData','{}'::jsonb)
    );
