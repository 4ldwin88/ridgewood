...
begin
  ...
  if coalesce((verification_input->>'verified')::boolean,false) is not true
     or coalesce((verification_input->>'userVerified')::boolean,false) is not true then
    raise exception 'fresh_verification_required';
  end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object('domain',d.domain_key,'readiness',d.readiness,'summary',d.summary) order by d.domain_key),'[]'::jsonb)
    into readiness_snapshot from public.predevelopment_domains d where d.project_state_id=ps.id;
  ...
  values(auth_id,ps.id,'approved',nullif(trim(authority_basis_input),''),readiness_snapshot,'{}'::jsonb,uid,now());
  ...
  values(ps.id,'project_authorized',uid,jsonb_build_object('workspaceId',ps.workspace_id,'authorizationRecordId',auth_id,'authorityBasis',nullif(trim(authority_basis_input),''),'verificationReference',verification_ref,'verificationMethod',verification_input->>'method','verifiedAt',verification_time),now());
