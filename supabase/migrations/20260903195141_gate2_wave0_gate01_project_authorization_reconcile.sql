...
begin
  ...
  values(auth_id,ps.id,'approved',nullif(trim(authority_basis_input),''),readiness_snapshot,coalesce(verification_input->'evidence','[]'::jsonb),uid,now());
  update public.project_states set stage='project_authorization_setup',commercial_stage='project_authorization_setup',updated_at=now() where id=ps.id returning * into ps;
  insert into public.audit_events(project_state_id,event_type,actor_user_id,payload,occurred_at)
  values(ps.id,'project_authorized',uid,jsonb_build_object('workspaceId',ps.workspace_id,'authorizationRecordId',auth_id,'authorityBasis',nullif(trim(authority_basis_input),''),'gate','gate_01_project_authorization_contract_commitment','verificationReference',verification_ref,'verificationMethod',verification_input->>'method','verifiedAt',verification_time,'nextStage','project_authorization_setup'),now());
  return ps;
end
$$;
comment on function public.authorize_project_state(uuid,text,jsonb) is 'Gate 01 Project Authorization / Contract Commitment. Requires explicit permission, scoped authority, fresh replay-protected verification, contextual Predevelopment resolution, and resolved Authorization requirements; advances the same Project State to Project Authorization & Setup.';
