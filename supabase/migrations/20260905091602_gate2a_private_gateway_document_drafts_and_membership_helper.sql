...
security invoker
set search_path = ''
as $$ select private.is_workspace_member(target_workspace_id); $$;
...
begin
  ...
  values(record_id,target_project_state_id,package_key_input,category_key_input,document_type_input,title_input,0,actor,pg_catalog.now(),pg_catalog.now(),ws);
  insert into public.document_revisions(id,document_record_id,revision_number,state,created_by,created_at,source_data)
  values(revision_id,record_id,1,'draft',actor,pg_catalog.now(),coalesce(initial_data,'{}'::jsonb));
...
security invoker
set search_path = ''
as $$ select private.create_project_state_document_draft_command(target_project_state_id,package_key_input,category_key_input,document_type_input,title_input,initial_data); $$;
...
security invoker
set search_path = ''
as $$ select private.update_project_state_document_draft_command(target_revision_id,source_data_input); $$;
