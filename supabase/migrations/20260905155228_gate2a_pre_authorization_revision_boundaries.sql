...
begin
 ...
 if ps.stage in ('predevelopment','authorization') then
   update public.project_state_predevelopment_domains set readiness=case when readiness='not_started' then readiness else 'in_progress' end,updated_at=pg_catalog.now() where project_state_id=ps.id;
 end if;
 return ps;
end $$;
