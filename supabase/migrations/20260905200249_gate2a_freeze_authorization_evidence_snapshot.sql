...
begin
  ...
  if ps.status <> 'active' or ps.archived_at is not null or ps.stage <> 'authorization' then raise exception 'project_authorization_not_allowed'; end if;
  ...
  if exists(select 1 from public.document_records dr join public.document_revisions rv on rv.document_record_id=dr.id where dr.project_state_id=ps.id and rv.state='draft' and rv.archived_at is null) then raise exception 'authorization_document_drafts_exist'; end if;
  ...
  select jsonb_build_object(
    'publishedDocuments',coalesce((select jsonb_agg(jsonb_build_object('documentRecordId',x.record_id,'revisionId',x.revision_id,'revisionNumber',x.revision_number,'documentType',x.document_type,'categoryKey',x.category_key,'title',x.title,'publishedAt',x.published_at,'publishedBy',x.published_by,'sourceSnapshot',x.published_source_snapshot) order by x.document_type,x.revision_number) from (select distinct on (dr.id) dr.id record_id,rv.id revision_id,rv.revision_number,dr.document_type,dr.category_key,dr.title,rv.published_at,rv.published_by,rv.published_source_snapshot from public.document_records dr join public.document_revisions rv on rv.document_record_id=dr.id where dr.project_state_id=ps.id and rv.state='published' and rv.archived_at is null order by dr.id,rv.revision_number desc) x),'[]'::jsonb),
    'evidenceReferences',coalesce((select jsonb_agg(jsonb_build_object('id',e.id,'title',e.title,'sourceSystem',e.source_system,'sourceUrl',e.source_url,'provenanceNote',e.provenance_note,'knowledgeState',e.knowledge_state,'createdAt',e.created_at) order by e.created_at) from public.evidence_references e where e.project_state_id=ps.id),'[]'::jsonb),
    ...
    'authorizationConditions',coalesce(verification_input->'conditions','[]'::jsonb),
