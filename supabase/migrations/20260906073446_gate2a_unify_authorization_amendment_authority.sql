alter table public.authorization_amendments add column if not exists document_record_id uuid references public.document_records(id), add column if not exists original_revision_id uuid references public.document_revisions(id), add column if not exists changed_fields jsonb;
