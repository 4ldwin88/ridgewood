...
begin
  ...
  select exists(select 1 from public.actions a where a.project_state_id = ps.id and a.lifecycle_stage = 'opportunity' and a.status in ('open','in_progress','blocked')) into has_open_action;
  update public.project_state_stage_requirements r set status = case r.requirement_key
    ...
    else r.status end, updated_by = uid, updated_at = pg_catalog.now()
