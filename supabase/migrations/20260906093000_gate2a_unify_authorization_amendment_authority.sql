...
alter table public.authorization_amendments
  drop constraint if exists authorization_amendments_changed_fields_check;
alter table public.authorization_amendments
  add constraint authorization_amendments_changed_fields_check
  check (changed_fields is null or (jsonb_typeof(changed_fields) = 'object' and changed_fields <> '{}'::jsonb));
...
create or replace function public.record_authorization_amendment(
  project_state_input uuid,
  amendment_type_input text,
  summary_input text,
  rationale_input text,
  authority_basis_input text default null,
  evidence_input jsonb default '[]'::jsonb,
  document_record_input uuid default null,
  original_revision_input uuid default null,
  changed_fields_input jsonb default null
)
...
declare
  ...
  ar public.authorization_records;
  result public.authorization_amendments;
...
begin
  ...
  select exists(
    select 1 from (
      ...
      where d.workspace_id = ps.workspace_id and d.grantee_user_id = uid and d.status = 'active'
        and d.authority_key = 'project.authorize' and d.revoked_at is null
  ...
  select * into ar from public.authorization_records
  where project_state_id = ps.id and outcome = 'approved'
  ...
  insert into public.authorization_amendments(project_state_id,authorization_record_id,amendment_type,summary,rationale,authority_basis,evidence_snapshot,actor_user_id,document_record_id,original_revision_id,changed_fields)
  values(ps.id,ar.id,amendment_type_input,pg_catalog.btrim(summary_input),pg_catalog.btrim(rationale_input),nullif(pg_catalog.btrim(authority_basis_input),''),coalesce(evidence_input,'[]'::jsonb),uid,document_record_input,original_revision_input,changed_fields_input)
  returning * into result;

  insert into public.audit_events(project_state_id,event_type,actor_user_id,payload,occurred_at)
  values(ps.id,'authorization_' || amendment_type_input || '_recorded',uid,jsonb_build_object('authorizationRecordId',ar.id,'authorizationAmendmentId',result.id,'documentRecordId',document_record_input,'originalRevisionId',original_revision_input,'changedFields',changed_fields_input,'summary',result.summary,'authorityBasis',result.authority_basis),pg_catalog.now());
  return result;
end;
$$;
