-- Human QA: completing the governed Opportunity action must not regress
-- "Next step identified" to not started. A recorded non-cancelled
-- Opportunity-scoped Action is evidence that the next step was identified.
...
begin
  ...
  update public.project_state_stage_requirements r set status = case r.requirement_key
    ...
    when 'next_step' then case when has_action then 'satisfied'::public.readiness_state else 'not_started'::public.readiness_state end
    else r.status end,
    updated_by = uid, updated_at = pg_catalog.now()
