-- Lock the editable revision before checking authority and saving.
-- A concurrent publish/discard rechecks the predicate after waiting, so a
-- stale save fails instead of reporting success and writing a false audit.
...
begin
  ...
  select d.workspace_id,d.project_state_id into ws,ps from public.document_revisions r join public.document_records d on d.id=r.document_record_id where r.id=target_revision_id and r.state='draft' and r.archived_at is null for update of r;
