-- Forward Gate 01 contract. Filename generated by Supabase CLI in CI run 34660598201.
-- No historical decisions are rewritten; no existing Project State is moved.
...
begin
  ...
  return jsonb_build_object('projectStateId',p.id,'version',coalesce(s.version,0),'authorizationRecordId',coalesce(s.authorization_record_id,a),
    ...
    'decisions',(select coalesce(jsonb_agg(jsonb_build_object('id',d.id,'disposition',d.disposition,'rationale',d.rationale,'actorUserId',d.actor_user_id,'createdAt',d.created_at,'obligations',d.obligations,'setupVersion',(select version from public.project_setup_versions v where v.id=d.setup_version_id),'authorityReference',d.authority_snapshot->>'owner_approval_reference','evidence',d.evidence_snapshot->'evidence') order by d.created_at desc),'[]') from public.project_gate01_decisions d where d.project_state_id=p.id));
end $$;
